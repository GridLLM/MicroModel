...
const loadMappings = (): WorkflowMappings => {
  try {
    ...
    if (!fs.existsSync(mappingsPath)) {
      console.error(
        "config.json file not found. Please create one in the project root.",
      );
      return {};
    }
    ...
    for (const [workflowId, config] of Object.entries(parsedMappings)) {
      if (typeof config !== "object" || config === null) {
        console.error(
          `Invalid configuration for workflow "${workflowId}". Expected object, got ${typeof config}.`,
        );
        return {};
      }
      ...
      if (
        !workflowConfig.OPENAI_API_HOST ||
        typeof workflowConfig.PORT !== "number"
      ) {
        console.error(
          `Invalid configuration for workflow "${workflowId}". Missing required fields: OPENAI_API_HOST, PORT.`,
        );
        return {};
      }
    }
...
const startServers = () => {
  ...
  if (Object.keys(mappings).length === 0) {
    console.error(
      "No mappings found in config.json. Please create a config.json file with your workflow configurations.",
    );
  ...
  Object.entries(mappings).forEach(([workflowId, config]) => {
    ...
    if (!config.OPENAI_API_HOST || typeof config.PORT !== "number") {
      console.error(
        `Invalid configuration for workflow "${workflowId}". Missing OPENAI_API_HOST or PORT.`,
      );
      process.exit(1);
    }
